...
@section('title', 'Case Matrix')
@section('page-title', 'Legal Case Matrix')
@section('page-subtitle', 'Excel-style case tracking and management system')
...
<div class="matrix-container">
    ...
    <div class="matrix-controls">
        ...
        <div class="matrix-filters-row">
            ...
            <a href="{{ route('cases.export.excel', request()->only(['status', 'search'])) }}" class="btn btn-excel btn-sm">
                <i class="bi bi-file-earmark-excel"></i> Export Excel
            </a>

            <a href="{{ route('cases.export.pdf', request()->only(['status', 'search'])) }}" class="btn btn-pdf btn-sm">
    ...
    <div class="matrix-table-wrapper">
        ...
        <table class="matrix-table">
            ...
            <tbody>
                ...
                <tr class="row-{{ $case->status }}" data-case-id="{{ $case->id }}">
                    <td class="cell-case-number" style="width: 100px;">{{ $case->case_number ?: '-' }}</td>
                    <td class="cell-bold-text" style="width: 130px;">{{ $case->docket_no ?: '-' }}</td>
                    <td class="cell-bold-text" style="width: 120px;">{{ $case->old_folder_no ?: '-' }}</td>
                    <td class="td-case-title" style="width: 300px;">{{ $case->case_title ?: '-' }}</td>
                    <td class="cell-bold-text" style="width: 180px;">{{ $case->client_name ?: '-' }}</td>
                    <td style="width: 120px;">{{ $case->case_type ?: '-' }}</td>
                    <td style="width: 120px;">{{ $case->section ?: '-' }}</td>
                    <td style="width: 120px;">{{ $case->court_agency ?: '-' }}</td>
                    <td style="width: 120px;">{{ $case->region ?: '-' }}</td>
                    <td class="cell-bold-text" style="width: 180px;">{{ $case->assigned_lawyer ?: '-' }}</td>
                    <td class="cell-bold-text" style="width: 180px;">{{ $case->handling_counsel_ncip ?: '-' }}</td>
                    <td style="width: 110px;">{{ $case->date_filed ? \Carbon\Carbon::parse($case->date_filed)->format('Y-m-d') : '-' }}</td>
                    <td style="width: 110px;">{{ $case->deadline_date ? \Carbon\Carbon::parse($case->deadline_date)->format('Y-m-d') : '-' }}</td>
                    <td style="width: 110px;">{{ $case->hearing_date ? \Carbon\Carbon::parse($case->hearing_date)->format('Y-m-d') : '-' }}</td>
                    <td class="cell-notes" style="width: 200px;" title="{{ $case->actions_taken }}">{{ Str::limit($case->actions_taken, 50) ?: '-' }}</td>
                    <td class="cell-notes" style="width: 200px;" title="{{ $case->action }}">{{ Str::limit($case->action, 50) ?: '-' }}</td>
                    <td class="cell-notes" style="width: 250px;" title="{{ $case->issues_grounds }}">{{ Str::limit($case->issues_grounds, 60) ?: '-' }}</td>
                    <td class="cell-notes" style="width: 200px;" title="{{ $case->prayers_relief }}">{{ Str::limit($case->prayers_relief, 50) ?: '-' }}</td>
                    <td class="cell-bold-text" style="width: 150px;">{{ $case->new_sc_no ?: '-' }}</td>
                    <td class="cell-notes" style="width: 150px;" title="{{ $case->remarks }}">{{ $case->remarks ?: '-' }}</td>
                    <td class="cell-notes" style="width: 200px;" title="{{ $case->notes }}">{{ Str::limit($case->notes, 50) ?: '-' }}</td>
                    ...
                    <td style="width: 170px;">
                        <div class="cell-actions">
                            <a href="{{ route('cases.show', $case) }}" class="cell-action-btn btn-view" title="View Case">
                                <i class="bi bi-eye-fill"></i>
                            </a>
                            <button type="button" class="cell-action-btn btn-edit" title="Edit Case" onclick="editCase({{ $case->id }})">
                                <i class="bi bi-pencil-fill"></i>
                            </button>
                            @if($case->status === 'completed')
                            <form action="{{ route('cases.reopenCase', $case) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('PATCH')
                            ...
                            @else
                            <form action="{{ route('cases.markAsCompleted', $case) }}" method="POST" style="display: inline;">
                                @csrf
                                @method('PATCH')
                            ...
                            @endif
                            <button type="button" class="cell-action-btn btn-delete" title="Delete" onclick="deleteCase({{ $case->id }})">
...
<div class="modal fade" id="caseModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <form id="caseForm" method="POST" action="{{ route('cases.store') }}" novalidate>
                @csrf
...
<div class="modal fade" id="deleteAllModal" tabindex="-1" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-danger">
            ...
            <form action="{{ route('cases.destroyAll') }}" method="POST" id="deleteAllForm">
                @csrf
                @method('DELETE')
                <div class="modal-body">
                    <div class="alert alert-danger border-danger mb-3">
                        ...
                        <p class="mb-2"><strong>This action will permanently delete ALL {{ $cases->count() }} case(s) in the system.</strong></p>
                        <hr>
...
<script>
    ...
    function openNewCaseModal() {
        ...
        document.getElementById('caseForm').action = '{{ route("cases.store") }}';
