...
<body>
    ...
    <div class="export-info">
        <strong>Export Date:</strong> {{ now()->format('F d, Y h:i A') }}<br>
        <strong>Total Cases:</strong> {{ $cases->count() }}
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ Str::limit($case->case_title, 60) }}</td>
                <td>{{ Str::limit($case->client_name, 30) }}</td>
                <td>{{ $case->section }}</td>
                <td>{{ $case->court_agency ?? 'N/A' }}</td>
                <td>{{ \Carbon\Carbon::parse($case->date_filed)->format('Y-m-d') }}</td>
                <td>{{ \Carbon\Carbon::parse($case->deadline_date)->format('Y-m-d') }}</td>
                <td style="text-align: center;">
                    <span class="priority-{{ strtolower($case->priority) }}">
                        {{ $case->days_remaining }} days
                    </span>
                </td>
                <td>
                ...
                <td>{{ $case->assigned_lawyer ?? 'N/A' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="footer">
        ...
        <p>&copy; {{ now()->year }} Legal Aid Office - All Rights Reserved</p>
    </div>
</body>
</html>
